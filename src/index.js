...
class PatientVisualizer extends React.Component {
  render() {
    ...
    const height_obs = searchableObs.find(
      (o) => o.code.coding[0].display === "Body Height"
    );
    const weight_obs = searchableObs.find(
      (o) => o.code.coding[0].display === "Body Weight"
    );
    ...
    if (patient.address[0].extension) {
      const geolocation = patient.address[0].extension.find(
        (e) => e.url === "http://hl7.org/fhir/StructureDefinition/geolocation"
      );

      if (geolocation && geolocation.extension.length > 1) {
        lat = geolocation.extension.find((e) => e.url === "latitude")
          .valueDecimal;
        lng = geolocation.extension.find((e) => e.url === "longitude")
          .valueDecimal;
      }
    }
    ...
    return (
      <div>
        <div className="health-record__header">
          <div className="header-title">Patient</div>
...
class GenericVisualizer extends React.Component {
  ...
  renderHeaderLine() {
    const columns = this.props.columns
      .filter(
        (c) => c.versions === "*" || c.versions.includes(this.props.version)
      )
      .map((c) => c.title);
  ...
  renderBodyLine(line) {
    ...
    if (this.props.nestedRows) {
      for (const nestedRow of this.props.nestedRows) {
        ...
        const subColumns = nestedRow.columns.filter(
          (c) => c.versions === "*" || c.versions.includes(this.props.version)
        );

        for (const subRowLine of subRowLines) {
          nestedRows.push(
            <tr key={nestedRow.keyFn(subRowLine)}>
              {subColumns.map((c, i) => {
